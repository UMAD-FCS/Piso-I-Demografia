...
base        <- readxl::read_excel("Data/Base_Motor_Demografica_rev2025_vf.xlsx")
...
##### **Población total censada (en miles de personas)** 
```{r}
...
```{r}
  t1 <-   base %>% filter(NOMINDICADOR == "Población total censada") %>%
    mutate(VALOR = round(as.numeric(VALOR)/1000,1)) %>%
    rename(ANIO = FECHA)
...
base        <- readxl::read_excel("Data/Base Motor Demografica_rev2025_vf.xlsx")
...
Los censos de población constituyen los pilares sobre los que se apoyan las proyecciones de población.  Para la realización de las proyecciones de población se utiliza la Ecuación Compensadora, que permite determinar el stock de población en un momento (Pf) como el stock en un momento  momento anterior (Pi), más los nacimientos (N) y la Inmigración (I), menos las defunciones (D) y la Emigración (E), ocurridas entre los dos momentos considerados.
...
<div style="text-align: center">
##### **Población censada y proyecciones de población (en miles de personas)** 
##### Total país, 1950-2020 
</div>
...
##### **Proyecciones del Instituto Nacional de Estadística y de Naciones Unidas (en miles de personas)** 
...
#===============================================================================
  g1 <- 
    base %>% filter((NOMINDICADOR == "Proyecciones de población (INE)" | NOMINDICADOR == "Proyecciones de población (NNUU, 2019)") & SEXO == "Total") %>%
    mutate(VALOR = as.numeric(VALOR)/1000) %>% 
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(group = 1, color = NOMINDICADOR)) +
    scale_color_manual(
...
```{r}
  t1 <-  base %>% filter((NOMINDICADOR == "Proyecciones de población (INE)" | NOMINDICADOR == "Proyecciones de población (NNUU, 2019)") & SEXO == "Total") %>%
    mutate(VALOR = round(as.numeric(VALOR)/1000,1)) %>%
    rename(ANIO = FECHA)
  t1 <- t1 [,c("NOMINDICADOR","PAIS","ANIO","VALOR")]
...
G <- 
    base %>% filter((NOMINDICADOR == "Proyecciones de población (INE)" | 
                    NOMINDICADOR == "Proyecciones de población (NNUU, 2019)")
                    & SEXO == "Total") %>%
    mutate(VALOR = as.numeric(VALOR)/1000) %>% 
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(group = 1, color = NOMINDICADOR)) +
...
```{r, out.width='100%'}
#===============================================================================
tabla_piramide <- base %>% filter(NOMINDICADOR == "Población por edades quinquenales" & SEXO != "Total")
tabla_piramide <- tabla_piramide %>% mutate(VALOR = as.numeric(VALOR))
tabla_piramide <- tabla_piramide %>% mutate(EDAD = fct_inorder(EDAD))

p2023 <- tabla_piramide %>% filter(FECHA == "2023")
p2023 <- sum(p2023$VALOR)
...
g1 <- tabla_piramide %>% ggplot(
  ...
  transition_time(as.numeric(FECHA)) +
  #labs(title = paste("Año:", c("1963","1975","1985","1996","2011")),
...
anim_save("images/g1.gif", duration = 5, fps = 20, g1)
...
tabla_piramide <- base %>% 
  filter(NOMINDICADOR == "Población por edades quinquenales" & SEXO != "Total")
tabla_piramide <- tabla_piramide %>% mutate(VALOR = as.numeric(VALOR))
tabla_piramide <- tabla_piramide %>% mutate(EDAD = fct_inorder(EDAD))
...
tabla_piramide <- tabla_piramide %>% 
  mutate(porcentaje_total = VALOR*100/ptot)
tabla_piramide <- tabla_piramide %>% 
  mutate(FECHA = as.factor(FECHA))
tabla_piramide <- tabla_piramide %>%
  mutate(porcentaje_total = as.numeric(porcentaje_total))
...
anim_save("images/g2.gif", duration = 5, fps = 20, g2)
...
anim_save("images/G.gif", duration = 5, fps = 20, G)
...
#===============================================================================
  g1 <- 
    base %>% filter((NOMINDICADOR == "Porcentaje de población menor de 15 años" | NOMINDICADOR == "Porcentaje de población de 15 a 64 años" | NOMINDICADOR =="Porcentaje de población de 65 y más años") & DEPARTAMENTO_UY=="Total") %>%
    mutate(VALOR = as.numeric(VALOR)) %>% 
    ggplot(aes(x = FECHA, y = VALOR)) +
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Porcentaje de población menor de 15 años" | NOMINDICADOR == "Porcentaje de población de 15 a 64 años" | NOMINDICADOR =="Porcentaje de población de 65 y más años") & DEPARTAMENTO_UY=="Total") %>%
    mutate(VALOR = round(as.numeric(VALOR),1)) %>%
    rename(ANIO = FECHA)
  t1 <- t1 [,c("NOMINDICADOR","PAIS","ANIO","VALOR")]
...
### Gráfico ###
 ...
 G <- 
    base %>% filter(NOMINDICADOR == "Índice de envejecimiento" | 
...
#===============================================================================
  g1 <- 
    base %>% filter((NOMINDICADOR == "Relación de dependencia total" | NOMINDICADOR == "Relación de dependencia de los niños" | NOMINDICADOR =="Relación de dependencia de la vejez") & DEPARTAMENTO_UY=="Total") %>%
    mutate(VALOR = as.numeric(VALOR)) %>% 
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Relación de dependencia total" | NOMINDICADOR == "Relación de dependencia de los niños" | NOMINDICADOR =="Relación de dependencia de la vejez") & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
### Gráfico ###
 ...
 G <- 
    base %>% filter((NOMINDICADOR == "Relación de dependencia total" | 
    ...
    scale_color_manual(
      values = c(639, "#a1dd70", "orangered"),
      labels = c("Relación de dependencia total", 
                 "Relación de dependencia de los niños", 
                 "Relación de dependencia de la vejez")) +
    ylim(0, 100) +
    xlim(1995, 2050) +
...
##### **Cantidad de nacimientos** 
```{r}
  periodo <- base[base$NOMINDICADOR == "Cantidad de nacimientos",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
  base %>% filter(NOMINDICADOR == "Cantidad de nacimientos") %>%
  mutate(VALOR = as.numeric(VALOR)) %>%
  ...
  ylim(0, 70000) +
  xlim(1995, max) +
  scale_x_continuous(breaks=seq(1995, max, 4)) +
  theme_minimal() +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Cantidad de nacimientos" & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
  ...
  ylim(0, 70000) +
  xlim(1995, max) +
  scale_x_continuous(breaks=seq(1995, max, 5)) +
  theme_minimal() +
  theme(legend.position = "none")+
  labs(
    title = "",
    x = "",
    y = "") +
  scale_x_continuous(breaks = c(1900, 1910,1920, 1930, 1940, 
...
ggplotly(G) %>% 
    config(displayModeBar = F) %>%
    layout(margin = list(b = 100), legend = list(x = 0.5, y = -0.1,
                                                 orientation = "h", 
                                                 xanchor = "center")) 
...
Fuente: Unidad de Métodos y Acceso a Datos (UMAD) (FCS-UdelaR) con base en Estadísticas vitales del Ministerio de Salud Pública e Instituto Nacional de Estadísticas (Proyecciones de Población 1996-2025. Rev. 2013)
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Tasa bruta de natalidad (por mil)" & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
ggplotly(G) %>% 
    config(displayModeBar = F) %>%
    layout(margin = list(b = 100), legend = list(x = 0.5, 
                                                 y = -0.1,
                                                 orientation = "h", 
                                                 xanchor = "center"))
...
La evolución reciente muestra que, a pesar del crecimiento del número de mujeres en edad fértil, desciende el número de nacimientos, lo que conduce a confirmar que lo que está descendiendo es el número de hijos que tienen las mujeres.
</div>
...
```{r}
  periodo <- base[base$NOMINDICADOR == "Cantidad de mujeres en edad fértil",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Cantidad de nacimientos" |NOMINDICADOR == "Cantidad de mujeres en edad fértil") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
    scale_color_manual(
      values = c(639, "#a1dd70"),
      labels = c("Nacimientos", "Mujeres en edad fértil")) +
    ylim(0, 900000) +
    xlim(1996, max) +
    theme_minimal() +
    labs(
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Cantidad de nacimientos" | NOMINDICADOR == "Cantidad de mujeres en edad fértil") & DEPARTAMENTO_UY=="Total" & FECHA>=1996)   %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
    base %>% filter(NOMINDICADOR == "Cantidad de nacimientos" |
                      NOMINDICADOR == "Cantidad de mujeres en edad fértil") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, 
               y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
...
##### **Tasa general de fecundidad**
```{r}
  periodo <- base[base$NOMINDICADOR == "Tasa General de Fecundidad",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
g1 <- 
    base %>% filter(NOMINDICADOR == "Tasa General de Fecundidad" ) %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Tasa General de Fecundidad" & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Tasas Específicas de Fecundidad por edad simple (por mil)"&FECHA==1996|FECHA==2000|FECHA==2005|FECHA==2010|FECHA==2015|FECHA==2019|FECHA==2023) %>%
  mutate(VALOR = as.numeric(VALOR)) %>%
  ...
  scale_color_manual(
       values = c("black", "gray88", "gray70", "gray60", "gray50", "gray40", "gray30"),
       labels = c("1996", "2000", "2005", "2010", "2015", "2019", "2023")) +
  ylim(0, 150) +
  xlim(10, 50) +
  theme_minimal() +
  labs(
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Tasas Específicas de Fecundidad por edad simple (por mil)") & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
  ...
  scale_color_manual(
       values = c("black", "gray88", "gray70", "gray60", "gray50", "gray40", "gray30"),
       labels = c("1996", "2000", "2005", "2010", "2015", "2019", "2022")) +
  ylim(0, 150) +
  xlim(10, 50) +
  theme_minimal() +
  labs(
...
<div style="text-align: justify">
La **tasa global de fecundidad** (TGF o TFR, por sus siglas en inglés, Total Fertility Rate) es el número promedio de hijos que una mujer habría tenido al final de sus años reproductivos si la misma se hubiera ajustado a las tasas de fecundidad por edad específica durante cada año de su vida reproductiva. La TGF es la medida de fecundidad más ampliamente utilizada por dos razones principales: 1) no se ve afectada por las diferencias o cambios en la composición de la población por edad y sexo y 2) proporciona una medida fácil de comprender de la fecundidad hipotética completa. 

El descenso de la fecundidad en los últimos años ha sido el más pronunciado de la historia del país, y ha puesto a Uruguay en la categoría de países de muy baja fecundidad (menos de 1.5 hijos por mujer).
</div>
...
<div style="text-align: justify">
El descenso de la fecundidad adolescente es responsable de una parte relevante del descenso de la fecundidad global.
</div>
...
```{r}
  periodo <- base[base$NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & base$DEPARTAMENTO_UY == "Total"&is.na(base$FECHA)==F,]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & DEPARTAMENTO_UY == "Total") %>%
 mutate(UNIDAD = case_when(NOMINDICADOR=="Tasa de fecundidad adolescente observada  (por mil) por departamento" ~ "Tasa de fecundidad adolescente")) %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = UNIDAD)) +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & DEPARTAMENTO_UY == "Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
    base %>% 
  filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento (1996-2021)" & DEPARTAMENTO_UY == "Total") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
...
Fuentes: Unidad de Métodos y Acceso a Datos (UMAD) (FCS-UdelaR) con base en Estadísticas vitales del Ministerio de Salud Pública 
...
<div style="text-align: center">
##### **Porcentaje de nacimientos de madres menores de 20 años sobre total de nacimientos**
```{r}
  periodo <- base[base$NOMINDICADOR == "Porcentaje de nacimientos de madres menores de 20 años sobre total de nacimientos",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Porcentaje de nacimientos de madres menores de 20 años sobre total de nacimientos" ) %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
    scale_color_manual(
      values = c (639),
      labels = c("Porcentaje de nacimientos de madres menores de 20 años sobre total de nacimientos")) +
    ylim(0, 30) +
    xlim(1998, max) +
    theme_minimal() +
    labs(
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Porcentaje de nacimientos de madres menores de 20 años sobre total de nacimientos") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
Fuente: Unidad de Métodos y Acceso a Datos (UMAD) (FCS-UdelaR) con base en Estadísticas vitales del Ministerio de Salud Pública
...
**Tasa de mortalidad:** La tasa de mortalidad (denominada también tasa bruta de mortalidad) mide la frecuencia de las defunciones ocurridas en un período en relación a la población total. Indica el número de muertes por cada 1.000 habitantes durante un año determinado. Es el cociente entre el número medio anual de defunciones ocurridas durante un período determinado y la población media de ese período. Las tasas brutas de mortalidad se ven afectadas por la estructura por edad de la población.

**Tasa de mortalidad por edad específica:** La tasa de mortalidad por edad específica corresponde al número de muertes ocurrido en un grupo de personas de cierta edad, dividido la cantidad de personas en dicha edad. Debido a que la mortalidad varía bastante según el sexo y la ascendencia étnico-racial, es factible calcular por separado las tasas de mortalidad por edad específica para hombres y mujeres y para grupos con distintas ascendencias étnico-raciales dentro de una población.
...
g1 <- 
    ...
    ylim(0, 15) +
    xlim(1895, max) +
    scale_x_continuous(breaks=seq(1895, max, 20)) +
    theme_minimal() +
    labs(
      color = "",
      title = "",
...
```{r}
  periodo <- base[base$NOMINDICADOR == "Cantidad de defunciones",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
  base %>% filter(NOMINDICADOR == "Cantidad de defunciones") %>%
  mutate(VALOR = as.numeric(VALOR)) %>%
  ggplot(aes(x = FECHA, y = VALOR, fill="NOMINDICADOR")) +
  geom_bar(stat="identity", position="stack") +
  scale_fill_manual(values=c(639)) +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Cantidad de defunciones") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
base %>% filter(NOMINDICADOR == "Esperanza de vida al nacer (1996-2050)") %>%
  ...
  ylim(0, 90) +
  xlim(1995, 2050) +
  scale_x_continuous(breaks=seq(1995, 2050, 10)) +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Esperanza de vida al nacer (1996-2050)" & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
g1 <- 
    ...
    mutate(UNIDAD=case_when(NOMINDICADOR == "Saldo migratorio" ~ "Saldo pasajeros uruguayos")) %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = UNIDAD)) +
    geom_point(aes(color = UNIDAD)) +
    scale_color_manual(
      values = c (639),
      labels = c("Saldo pasajeros uruguayos")) +
    ylim(-30000, 15000) +
    xlim(1975, max) +
    theme_minimal() +
    labs(
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Saldo migratorio" & DEPARTAMENTO_UY=="Total") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
#===============================================================================
  g1 <- 
    base %>% filter((NOMINDICADOR == "Cantidad de residencias otorgadas" | NOMINDICADOR == "Cantidad de resdencias definitivas otorgadas" | NOMINDICADOR =="Cantidad de resdencias temporarias otorgadas"|NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas") & DEPARTAMENTO_UY=="Total") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
  ...
  NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas" ~ "Fronterizas")) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = RESIDENCIA)) +
    geom_point(aes(color = RESIDENCIA)) +
    scale_color_manual(
      values = c(639, "#a1dd70", "orangered", "darkorange"),
      labels = c("Total", "Temporarias", "Definitivas", "Fronterizas")
    ) +
    ylim(0, 10000) +
    xlim(2013, max) +
  scale_x_continuous(breaks=seq(2013, max, 1)) +
    theme_minimal() +
    labs(
      color = "",
      title = "",
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Cantidad de residencias otorgadas" | NOMINDICADOR == "Cantidad de resdencias definitivas otorgadas" | NOMINDICADOR =="Cantidad de resdencias temporarias otorgadas") & DEPARTAMENTO_UY=="Total"|NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
  g1 <- 
    base %>% 
  filter((NOMINDICADOR == "Cantidad de residencias otorgadas" | 
            NOMINDICADOR == "Cantidad de resdencias definitivas otorgadas" | 
            NOMINDICADOR =="Cantidad de resdencias temporarias otorgadas"|
            NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas") & 
           DEPARTAMENTO_UY=="Total") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    mutate(RESIDENCIA = case_when(
  NOMINDICADOR =="Cantidad de residencias otorgadas" ~ "Total",
...
Nota: A partir de 2020, además de las residencias temporarias y definitivas concedidas, se incluye la cantidad de residencias fronterizas. 
</font>
</div>
...
Acceda a nuestro sistema de consultas personalizado de indicadores demográficos en el siguiente [link](https://bancodedatos-fcs.shinyapps.io/appDemografia/).
