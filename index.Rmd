...
base        <- readxl::read_excel("Data/Base Motor Demografica.xls")
...
tabla_piramide <- base %>% 
  filter(NOMINDICADOR == "Población por edades quinquenales" & SEXO != "ambos sexos")
tabla_piramide <- tabla_piramide %>% mutate(VALOR = as.numeric(VALOR))
tabla_piramide <- tabla_piramide %>% mutate(EDAD = fct_inorder(EDAD))
...
##### **Cantidad de nacimientos** 
```{r}
  periodo <- base[base$NOMINDICADOR == "Nacimientos",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
  base %>% filter(NOMINDICADOR == "Nacimientos") %>%
  mutate(VALOR = as.numeric(VALOR)) %>%
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Nacimientos" & DEPARTAMENTO_UY=="Total país") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Nacimientos" |NOMINDICADOR == "Mujeres en edad fértil") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Nacimientos" | NOMINDICADOR == "Mujeres en edad fértil") & DEPARTAMENTO_UY=="Total país" & FECHA>=1996)   %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
    base %>% filter(NOMINDICADOR == "Nacimientos" |
                      NOMINDICADOR == "Mujeres en edad fértil") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, 
               y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
...
```{r}
  periodo <- base[base$NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & base$DEPARTAMENTO_UY == "Total país"&is.na(base$FECHA)==F,]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
    base %>% filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & DEPARTAMENTO_UY == "Total país") %>%
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento" & DEPARTAMENTO_UY == "Total país") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
G <- 
    base %>% 
  filter(NOMINDICADOR == "Tasa de fecundidad adolescente observada  (por mil) por departamento (1996-2021)" & DEPARTAMENTO_UY == "Total país") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
    ggplot(aes(x = FECHA, y = VALOR)) +
    geom_line(aes(color = NOMINDICADOR)) +
    geom_point(aes(color = NOMINDICADOR)) +
...
```{r}
  periodo <- base[base$NOMINDICADOR == "Esperanza de vida al nacer",]
  max<- max(periodo$FECHA)
  min<- min(periodo$FECHA)
  periodo <- c(min," - ", max)
  periodo_formatted <- paste(
...
#===============================================================================
g1 <- 
base %>% filter(NOMINDICADOR == "Esperanza de vida al nacer") %>%
  mutate(VALOR = as.numeric(VALOR)) %>% 
  ggplot(aes(x = FECHA, y = VALOR)) +
...
```{r}
  t1 <- base %>% filter(NOMINDICADOR == "Esperanza de vida al nacer" & DEPARTAMENTO_UY=="Total país") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
...
#===============================================================================
  g1 <- 
    base %>% filter((NOMINDICADOR == "Cantidad de residencias otorgadas" | NOMINDICADOR == "Cantidad de resdencias definitivas otorgadas" | NOMINDICADOR =="Cantidad de resdencias temporarias otorgadas"|NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas") & DEPARTAMENTO_UY=="Total país") %>%
    mutate(VALOR = as.numeric(VALOR)) %>%
...
```{r}
  t1 <- base %>% filter((NOMINDICADOR == "Cantidad de residencias otorgadas" | NOMINDICADOR == "Cantidad de resdencias definitivas otorgadas" | NOMINDICADOR =="Cantidad de resdencias temporarias otorgadas") & DEPARTAMENTO_UY=="Total país"|NOMINDICADOR=="Cantidad de residencias fronterizas otorgadas") %>% 
    mutate(VALOR = round(as.numeric(VALOR),1)) %>% 
    rename(ANIO = FECHA)
